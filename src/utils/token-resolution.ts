import type { TokenCollection, TokenSchema, W3CToken } from '../types';

const ALIAS_REGEX = /\{([^}]+)\}/g;
const MATH_REGEX = /^[\d\s\.\+\-\*\/()]+$/;

export const resolveTree = (tokens: any, rootScope: any): any => {
  const resolveValue = (value: any, currentScope: any): any => {
    if (typeof value === 'object' && value !== null) {
      ...
      return resolved;
    } else if (value && typeof (value as W3CToken<any>).value !== 'undefined') {
      // Handle W3C token format
      return (value as W3CToken<any>).value;
    } else {
      return value;
    }
  };
...
export const convertW3CToTokenCollection = (schema: TokenSchema): TokenCollection => {
  ...
  if (schema.global?.color) {
    ...
    for (const [key, token] of Object.entries(schema.global.color)) {
      collection.colors[key] = typeof token === 'object' && token.value !== undefined ? (token.value as unknown) as string : (token as unknown) as string;
    }
  }
  ...
  if (schema.global?.typography) {
    ...
    if (schema.global.typography.fontFamily) {
      ...
      for (const [key, token] of Object.entries(schema.global.typography.fontFamily)) {
        collection.typography.fontFamily[key] = typeof token === 'object' && token.value !== undefined ? (token.value as unknown) as string : (token as unknown) as string;
      }
    }
  ...
  if (schema.global?.opacity) {
    ...
    for (const [key, token] of Object.entries(schema.global.opacity)) {
      const value = typeof token === 'object' && token.value !== undefined ? token.value : token;
      collection.opacity[key] = typeof value === 'number' ? value : parseFloat((value as unknown as string).toString());
    }
  }
  ...
  if (schema.global?.zIndex) {
    ...
    for (const [key, token] of Object.entries(schema.global.zIndex)) {
      const value = typeof token === 'object' && token.value !== undefined ? token.value : token;
      collection.zIndex[key] = typeof value === 'number' ? value : parseFloat((value as unknown as string).toString());
    }
  }
...
export const cleanRawTokens = (tokens: TokenSchema): TokenSchema => {
  const cleaned: TokenSchema = { ...tokens };
  ...
  if (cleaned.global) {
    ...
    if (cleaned.global.typography) {
      for (const [key, token] of Object.entries(cleaned.global.typography)) {
        if (token && typeof token.value !== 'undefined') {
          (cleaned.global.typography as any)[key] = resolveValue(token, cleaned.global);
        }
      }
    }
  }
