...
export const buildMantineConfig = (globals) => {
...
${Object.entries(colors)
        .map(([k, v]) => `    "${k}": [${v.map(color => `"${color}"`).join(', ')}]`)
        .join(',\n')}
  },
...
export const buildMantineTypes = (globals) => {
    const colorMap = globals.colors || {};
    const colors = Object.entries(colorMap)
        .filter(([_, v]) => typeof v === 'string' && v.startsWith('#'))
        .map(([k, v]) => '  ' + k + ': string;')
...
//# sourceMappingURL=mantine-builder.js.map